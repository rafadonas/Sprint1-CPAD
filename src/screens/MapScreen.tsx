...
export const MapScreen = ({ ocorrencias, onSelectOccurrence, onNavigateToReport }: MapScreenProps) => {
  ...
  return <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}><View style={styles.mapMock}>
    ...
    {filtered.map((item, index) => <TouchableOpacity key={item.id} style={[styles.marker, { top: 245 + (index % 3) * 105, left: 36 + (index % 4) * 74, backgroundColor: RISK_COLORS[item.risco] }]} onPress={() => onSelectOccurrence(item)} accessibilityRole="button" accessibilityLabel={`${item.local}, risco ${riskLabel(item.risco)}`}><MaterialCommunityIcons name="leaf" size={19} color="#fff" /></TouchableOpacity>)}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#08110C' }, mapMock: { flex: 1, backgroundColor: '#111', position: 'relative' }, mapImage: { ...StyleSheet.absoluteFill, width: '100%', height: '100%', opacity: 0.47 },
  ...
  legend: { position: 'absolute', top: 185, right: 14, backgroundColor: 'rgba(8,17,12,0.92)', padding: 10, borderRadius: 12, borderWidth: 1, borderColor: '#294133', zIndex: 10 }, legendTitle: { color: '#F8FAFC', fontSize: 11, fontWeight: '700', marginBottom: 7 }, legendItem: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 }, legendColor: { width: 9, height: 9, borderRadius: 5, marginRight: 7 }, legendLabel: { color: '#CBD5E1', fontSize: 10 },
  marker: { position: 'absolute', width: 39, height: 39, borderRadius: 20, alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: '#F8FAFC', elevation: 5, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.4, shadowRadius: 5 }, noResults: { position: 'absolute', left: 20, right: 20, top: 260, backgroundColor: 'rgba(8,17,12,0.96)', borderRadius: 18, zIndex: 20 }, reportButton: { position: 'absolute', left: 20, bottom: 104, backgroundColor: '#4ADE80', borderRadius: 16, paddingHorizontal: 18, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', elevation: 4 }, reportText: { color: '#07120A', fontWeight: '800', marginLeft: 7 },
});
